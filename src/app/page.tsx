...
export default function IntroPage() {
  return (
    <div className="relative h-screen w-screen overflow-hidden flex flex-col items-center justify-between bg-[#131313] text-[#e2e2e2] select-none">
      ...
      {/* Top HUD Header */}
      <header className="relative z-10 w-full px-6 py-4 flex justify-between items-center border-b-[3px] border-black bg-[#131313]/90 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <div className="w-3 h-3 bg-[#77da9f] rounded-full animate-ping" />
          <span className="font-mono text-xs font-bold text-[#77da9f] uppercase tracking-wider">
            SYSTEM: ONLINE
          </span>
        </div>

        <div className="flex items-center gap-4 font-mono text-xs text-[#c6c6c6]">
          <span className="border-[2px] border-black bg-[#1a1a1a] px-3 py-1 text-[#ffb2bd] font-bold">
            PROTOCOL_v1.0.0
          </span>
        </div>
      </header>
      ...
      <footer className="relative z-10 w-full bg-[#1a1a1a] border-t-[3px] border-black py-3 px-4 font-mono text-xs text-[#c6c6c6] overflow-hidden">
        <div className="flex justify-around items-center max-w-6xl mx-auto gap-4 font-bold flex-wrap">
          ...
          <span className="hidden sm:inline text-black">&bull;</span>
          <span className="flex items-center gap-2 text-[#77da9f]">
            <span>📊</span> SMART BUDGET PROTOCOLS
          </span>
          <span className="hidden sm:inline text-black">&bull;</span>
          <span className="flex items-center gap-2 text-[#ffb2bd]">
